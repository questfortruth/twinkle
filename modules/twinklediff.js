...
Twinkle.diff = function twinklediff() {
	...
	Twinkle.addPortletLink( mw.util.wikiScript("index")+ "?" + $.param( query ), '最後', 'tw-lastdiff', '顯示最後修改' );
	...
	if (Morebits.queryString.exists('diff')) {
		...
		Twinkle.addPortletLink( mw.util.wikiScript("index")+ "?" + $.param( query ), '當前', 'tw-curdiff', '顯示與當前版本間的差異' );
	}
};
