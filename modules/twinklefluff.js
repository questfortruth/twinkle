...
Twinkle.fluff = {
	...
	normal: function() {
		...
		if( mw.config.get('wgNamespaceNumber') === -1 && mw.config.get('wgCanonicalSpecialPageName') === "Contributions" ) {
			...
			var username = decodeURIComponent(/wiki\/Special:%E6%97%A5%E5%BF%97\/(.+)$/.exec($('#contentSub').find('a[title^="Special:日誌"]').last().attr("href").replace(/_/g, "%20"))[1]);
...
Twinkle.fluff.revert = function revertPage( type, vandal, autoRevert, rev, page ) {
	...
	var wikipedia_api = new Morebits.wiki.api( '抓取較早修訂版本信息', query, Twinkle.fluff.callbacks.main );
...
Twinkle.fluff.revertToRevision = function revertToRevision( oldrev ) {
	...
	var wikipedia_api = new Morebits.wiki.api( '抓取較早修訂版本信息', query, Twinkle.fluff.callbacks.toRevision.main );
...
Twinkle.fluff.callbacks = {
	toRevision: {
		main: function( self ) {
			...
			var wikipedia_api = new Morebits.wiki.api( '保存回退內容', query, Twinkle.fluff.callbacks.complete, self.statelem);
	...
	main: function( self ) {
		...
		if( revs.length < 1 ) {
			self.statelem.error( '沒有其它修訂版本，無法回退' );
			return;
		}
		var top = revs[0];
		if( lastrevid < self.params.revid ) {
			Morebits.status.error( '錯誤', [ '從服務器取得的最新修訂版本ID ', Morebits.htmlNode( 'strong', lastrevid ), ' 小於目前所顯示的修訂版本ID。這可能意味著當前修訂版本已被刪除、服務器延遲、或抓取到了壞掉的信息。取消。' ] );
			return;
		}
		var index = 1;
		if( self.params.revid !== lastrevid  ) {
			...
			if( lastuser === self.params.user ) {
				...
				case 'vand':
					...
					break;
				case 'agf':
					Morebits.status.warn( '警告', [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', self.params.user ) , ' 做出，因我們假定善意，取消回退操作，因為問題可能已被修復。' ]);
					return;
				default:
				...
				Morebits.status.error( '錯誤', [ '最新修訂版本由 ', Morebits.htmlNode( 'strong', lastuser ), ' 做出，所以這個修訂版本可能已經被回退了，取消回退操作。'] );
				return;
			}
		...
		if( Twinkle.fluff.whiteList.indexOf( self.params.user ) !== -1  ) {
			...
			case 'norm':
				/* falls through */
			default:
				var cont = confirm( '選擇了常規回退，但最新修改是由一個可信的機器人（' + self.params.user + '）做出的。您是否想回退前一個修訂版本？' );
				if( cont ) {
		...
		case 'vand':
			...
			summary = "回退[[Special:Contributions/" +
				self.params.user + "|" + self.params.user + "]] ([[User talk:" + self.params.user + "|討論]])" +
		...
		if (Twinkle.getPref('confirmOnFluff') && !userHasAlreadyConfirmedAction && !confirm("回退頁面：您確定嗎？")) {
			self.statelem.error( '用戶取消操作。' );
			return;
		}
		...
		if( (!self.params.autoRevert || Twinkle.getPref('openTalkPageOnAutoRevert')) &&
			...
			Morebits.status.info( '信息', [ '打開用戶 ', Morebits.htmlNode( 'strong', self.params.user ), ' 的對話頁' ] );
	...
	complete: function (apiobj) {
		...
		if (blacklist) {
			...
			apiobj.statelem.warn("要回退到的版本與當前版本相同，沒什麼要做的");
		} else {
...
Twinkle.fluff.formatSummary = function(builtInString, userName, userString) {
	...
	if (resultLen + contribsLen <= 255) {
		var talkLink = " ([[User talk:" + userName + "|討論]])";
